...
class ViewController: UIViewController, ESTBeaconManagerDelegate {
    ...
    func textForPromimity(proximity:CLProximity) -> (NSString)
    {
        ...
        case .Far:
            println("Far")
            distance = "far"
            beaconLabel.textColor = UIColor.whiteColor()
            return distance
        case .Near:
            println("Near")
            distance = "Near"
    ...
    func beaconManager(manager: ESTBeaconManager!, didEnterRegion region: ESTBeaconRegion!) {
        ...
        notification.alertBody = "You're flying through the danger zone."
        notification.soundName = "Default.mp3"
        println("You're flying through the danger zone.")
    ...
    func beaconManager(manager: ESTBeaconManager!, didExitRegion region: ESTBeaconRegion!) {
        ...
        var notification : UILocalNotification = UILocalNotification()
        notification.alertBody = "You've left the danger zone."
        notification.soundName = "Default.mp3"
        println("You're flying through the danger zone.")
        UIApplication.sharedApplication().presentLocalNotificationNow(notification)
    }
    ...
    @IBAction func getConnected(sender: AnyObject) {
        ...
        let url = NSURL(string: "https://dl.dropboxusercontent.com/u/36083021/alanfalconryco.png")
